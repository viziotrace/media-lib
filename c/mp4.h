...
// Add this structure for chunk-to-sample mapping
typedef struct
{
...
typedef struct
{
    ...
    // Sample mapping info
    uint64_t *chunk_offsets; // Rename from sample_offsets
    uint32_t chunk_count;    // Add chunk count
    uint32_t *sample_sizes;
    ChunkToSampleEntry *stsc_entries; // Add chunk-to-sample entries
    uint32_t stsc_entry_count;        // Add entry count
} MP4Context;
